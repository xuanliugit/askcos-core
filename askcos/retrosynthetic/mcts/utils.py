...
def nx_graph_to_paths(tree, root, max_depth=None, max_trees=None,
                      ...
                      pathway_ranker=None, update=False, **kwargs):
    """
    ...
    Args:
        ...
        update (bool, optional): whether to perform an additional update step
    ...
    if update:
        ...
        print("=== found %d pathways (overcounting duplicate templates)" % tree.nodes[root]['pathway_count'])
        print("=== min price: %.1f" % tree.nodes[root]['ppg'])
...
def prune(tree, root, max_prunes=100):
    ...
    #currently not removing nodes with in-degree=0 hoping this is not a problem
    for i in range (max_prunes):
        non_terminal_leaves = list((v for v, d in pruned_tree.out_degree() if d == 0 and not pruned_tree.nodes[v]['terminal']))

        for leaf in non_terminal_leaves:
            reactions = list(pruned_tree.predecessors(leaf))
            for rxn in reactions:
                pruned_tree.remove_node(rxn)
            pruned_tree.remove_node(leaf)
        if pruned_tree.number_of_nodes() == num_nodes:
            break
        else:
            num_nodes = pruned_tree.number_of_nodes()
    print ('{} round(s) of pruning were performed on the tree'.format(i+1))
    return pruned_tree
...
def full_update(tree, chem_smi, max_depth=None, depth=0, path=None):
        ...
        for reaction_node in tree.successors(chem_smi):
            ...
            for smi in precursors:
                full_update(tree, smi, depth=depth+1, max_depth=max_depth,  path=path+[chem_smi])
            ...
            if all([price > 0 for price in price_list]):
                ...
                rxn['pathway_count'] = np.prod([tree.nodes[smi]['pathway_count'] for smi in precursors])
